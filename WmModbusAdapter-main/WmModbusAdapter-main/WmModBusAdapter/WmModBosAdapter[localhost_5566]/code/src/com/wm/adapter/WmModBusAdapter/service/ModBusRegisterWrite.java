...
public class ModBusRegisterWrite extends WmAdapterService implements WmModBusAdapterConstants {
	...
	public WmRecord execute(WmManagedConnection connection, WmRecord input)
		...
		Integer RegisterLocation=null;
		try {
			if (mainCursor.first("SingleRegisterLocation")) {
				RegisterLocation = IDataUtil.getInt(mainCursor, "SingleRegisterLocation",1);
			}
		} catch (Throwable t) {};

		try {
			if (mainCursor.first("SingleRegisterValue")) {
				RegisterValue = IDataUtil.getString(mainCursor, "SingleRegisterValue");
			...
			try {
				...
				success= modbusConn.writeSingleRegisterModBusServer(RegisterLocation, RegisterValue);

				//WmModBosAdapter.retrieveLogger().logDebug(9999,"<"+RegisterValue+"> written");
			} catch (Throwable t) {
				success = false;
				modbusConn.destroy();
			}
