...
public class ModBusRegisterRead extends WmAdapterService implements WmModBusAdapterConstants {

	/**
	 * 
	 */
	private static final long serialVersionUID = 2805933730556581124L;
	...
	public WmRecord execute(WmManagedConnection connection, WmRecord input)
		...
		Integer coilQuantity=null;
		Integer coilStartingLocation=null;
		try {
			if (mainCursor.first("StartingHoldingRegisterAddress")) {
				coilStartingLocation = IDataUtil.getInt(mainCursor, "StartingHoldingRegisterAddress",1);
			}
		} catch (Throwable t) {};

		try {
			if (mainCursor.first("Quantity")) {
				coilQuantity = IDataUtil.getInt(mainCursor, "Quantity",1);
			...
			try {
				...
				success= modbusConn.readRegistersModBusServer(coilStartingLocation, coilQuantity);

				//WmModBosAdapter.retrieveLogger().logDebug(9999,"<"+success+"> read");
			} catch (Throwable t) {
				success = null;
				modbusConn.destroy();
			}
			...
			if (success==null)
			{
				...
				for(int i=0;i<success.length;i++) {
					IDataUtil.put(mainCursor, i+"", ""+success[i]);
				}}
