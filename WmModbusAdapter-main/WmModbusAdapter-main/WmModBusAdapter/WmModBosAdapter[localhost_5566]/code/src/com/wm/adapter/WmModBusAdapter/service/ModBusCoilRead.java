...
public class ModBusCoilRead extends WmAdapterService implements WmModBusAdapterConstants {
	...
	public WmRecord execute(WmManagedConnection connection, WmRecord input)
		...
		try {
			if (mainCursor.first("StartingCoilAddress")) {
				coilStartingLocation = IDataUtil.getInt(mainCursor, "StartingCoilAddress",1);
			}
		} catch (Throwable t) {};
		...
		try {
			...
			try {
				...
				success= modbusConn.readCoilsModBusServer(coilStartingLocation, coilQuantity);
